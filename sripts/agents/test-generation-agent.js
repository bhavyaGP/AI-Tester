...
// If this file is executed directly, run a simple generation flow using git changes
if (require.main === module) {
  (async () => {
    try {
      ...
      // Detect changes and build a list of files to generate tests for
      const changes = await git.analyzeChanges();
      const files = [];
      if (Array.isArray(changes.newFiles)) files.push(...changes.newFiles);
      if (Array.isArray(changes.rewrittenFiles)) files.push(...changes.rewrittenFiles);
      if (Array.isArray(changes.modifiedFiles)) files.push(...changes.modifiedFiles.map(f => (f.file ? f.file : f)));

      if (files.length === 0) {
        console.log('\u2139\ufe0f No changed files detected for test generation');
        process.exit(0);
      }
      ...
      for (const filePath of files) {
        try {
          ...
          if (!fm.isValidSourceFile(filePath)) {
            console.log(`\u23ed\ufe0f Skipping (not a valid source file): ${filePath}`);
            continue;
          }

          const testContent = await agent.generateTests(filePath);
          if (testContent) {
            await fm.writeTestFile(filePath, testContent);
            console.log(`\u2705 Tests generated for: ${filePath}`);
            generated++;
          }
        } catch (err) {
          console.error(`\u26a0\ufe0f Failed to generate tests for ${filePath}:`, err && err.message ? err.message : err);
        }
      }

      if (generated === 0) {
        console.log('\u2139\ufe0f No tests were generated by the test-generation agent');
      }
